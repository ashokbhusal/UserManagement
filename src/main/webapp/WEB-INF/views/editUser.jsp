		...
		<form action="update_user" method="post">
			...
			<div>
				<label>Gender</label> <input type="radio" name="gender" value="Male"
					...
					${user.gender=='Female'?'checked':''}>Female <input
			...
			<div>
				...
				<c:if test ="${fn:contains(user.hobbies, 'Hiking')}">checked</c:if>>Hiking
			</div>
			<div>
				<label>Nationality</label> <select name="nationality">

					<option value="Nepalese"
						${user.nationality=='Nepalese'?'selected':''}>Nepalese</option>
					<option value="American"
						${user.nationality=='American'?'selected':''}>American</option>
					<option value="Italian"
						${user.nationality=='Italian'?'selected':''}>Italian</option>
				</select>
			</div>
			<div>
